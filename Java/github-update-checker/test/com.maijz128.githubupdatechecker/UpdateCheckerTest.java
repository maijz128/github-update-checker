...
//    }
    ...
    public void getLatestReleases() throws Exception {
        ...
        UpdateChecker.LatestReleases latest = UpdateChecker.GetLatestReleases(HTML);

        assertEquals(latest.html_url, "releases/tag/0.0.1");
        assertEquals(latest.tag_name, "0.0.1");
        assertEquals(latest.assets.size(), 1);

        UpdateChecker.LatestReleases.Assets assets = latest.assets.get(0);
        assertEquals(assets.name, "0.0.1.zip");
        assertEquals(assets.browser_download_url, "/releases/download/0.0.1/0.0.1.zip");
    }
