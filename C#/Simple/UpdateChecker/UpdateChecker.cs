        ...
        public class LatestReleases
        {
            ...
            public static LatestReleases GetLatestReleases(string html)
            {
                ...
                try
                {
                    ...
                    Match match_body = Regex.Match(html, pattern_body);

                    result.tag_name = match_tag.Groups[1].Value;
                    result.name = match_name.Groups[1].Value;
                    result.body = match_body.Groups[1].Value;
                }
                catch (Exception  e)
                {
